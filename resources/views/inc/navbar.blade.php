    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse justify-content-between" id="navbarNavAltMarkup">
            <div class="navbar-nav">
                ...
                <a class="nav-link" href="/addUser" style="font-size: 30px; padding-right: 20px;">Medewerkers</a>
                @endif
            </div>
                <div class="navbar-nav">
                @if(isset(Auth::user()->username))
                <strong style="color: gray; padding: 0.5rem 0.1rem; font-size: 30px;">Welcome {{ Auth::user()->username }}</strong>
                <a class="nav-link float-end"  href="{{ url('/login/logout') }}" style="font-size: 30px;">Uitloggen</a>
                @endif
            </div>
        </div>
    </div>
